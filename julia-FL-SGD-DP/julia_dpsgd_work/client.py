...
class FlowerClient(NumPyClient):
    ...
    def _expected_cache_metadata(self) -> dict:
        metadata = {
            ...
            "embedding_normalization_eps": 1e-6,
        }
    ...
    @staticmethod
    def _normalize_embeddings(embeddings: torch.Tensor):
        eps = 1e-6
        mean = embeddings.mean(dim=0, keepdim=True)
        std = embeddings.std(dim=0, keepdim=True, unbiased=False).clamp_min(eps)
        normalized_embeddings = (embeddings - mean) / std
        return normalized_embeddings, mean, std

    @staticmethod
    def _denormalize_embeddings(embeddings: torch.Tensor, mean: torch.Tensor, std: torch.Tensor):
    ...
    def _build_or_load_synthetic_loader(self):
        ...
        real_embeddings_norm, embedding_mean, embedding_std = self._normalize_embeddings(real_embeddings)
        ...
        synthetic_embeddings = self._denormalize_embeddings(
        ...
        metadata = {
            **expected_metadata,
            ...
            "real_embedding_mean": float(real_embeddings.mean().item()),
            "real_embedding_std": float(real_embeddings.std(unbiased=False).item()),
            "normalized_embedding_mean": float(real_embeddings_norm.mean().item()),
            "normalized_embedding_std": float(real_embeddings_norm.std(unbiased=False).item()),
            "synthetic_embedding_mean": float(synthetic_embeddings.mean().item()),
            "synthetic_embedding_std": float(synthetic_embeddings.std(unbiased=False).item()),
        }
